...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    this.plane.scale.y = (this.viewport.height * (1125 * this.scale)) / this.screen.height;
    this.plane.scale.x = (this.viewport.width * (875 * this.scale)) / this.screen.width;
...
class CircularGalleryApp {
  ...
  onTouchUp(e) {
    ...
    const distance = Math.sqrt(Math.pow(endX - this.start, 2) + Math.pow(endY - this.startY, 2));
  ...
  onClick(x, y) {
    ...
    const mouse = {
      x: (x / this.screen.width) * 2 - 1,
      y: -(y / this.screen.height) * 2 + 1
    };

    // Very simple hit detection for circular gallery
    // Since it's a 3D gallery, we'd normally use Raycasting, 
    // but we can estimate based on the scroll position and item width.
    const width = this.medias[0].width;
    const viewportWidth = this.viewport.width;
    
    // Find the item closest to the click point in 3D space
    // For simplicity in this specific 3D setup, we can find which media is 'centered' or near the mouse.x
    // A better way is to check the plane positions.
    let closestMedia = null;
    let minDistance = Infinity;

    this.medias.forEach(media => {
      // Projected x position approximately
      const mediaX = media.plane.position.x;
      const projectedX = (mediaX / (this.viewport.width / 2)); 
      const dist = Math.abs(projectedX - mouse.x);
    ...
    if (closestMedia && minDistance < 0.5) {
      const lightbox = document.getElementById('lightbox');
      const lightboxImg = document.getElementById('lightbox-img');
      if (lightbox && lightboxImg) {
        lightbox.classList.add('active');
        lightboxImg.src = closestMedia.image;
